	...
	{#if loading}
		<div class="p-8" style="border: 1px solid var(--stroke);">
			<div class="flex items-center justify-center gap-3">
				...
				></span>
				<span class="text-sm" style="color: var(--text-tertiary);">Loading IP sets...</span>
			</div>
		</div>
					...
					<tbody>
						{#if ipset.lists.length > 0}
							<tr style="border-bottom: 1px solid var(--stroke);">
								<td class="px-5 py-3 whitespace-nowrap text-[11px] font-semibold uppercase" style="color: var(--text-quad); letter-spacing: 0.05em; width: 140px;">
									Lists
								</td>
								<td class="px-5 py-3">
									<div class="flex flex-wrap gap-1.5">
										{#each ipset.lists as list}
											<a
												href="/lists/{list}"
												class="inline-flex items-center px-2.5 py-0.5 text-xs font-medium transition-colors"
						...
						{#if ipset.routing.interfaces.length > 0}
							<tr style="border-bottom: 1px solid var(--stroke);">
								<td class="px-5 py-3 whitespace-nowrap text-[11px] font-semibold uppercase" style="color: var(--text-quad); letter-spacing: 0.05em;">
									Interfaces
								</td>
								<td class="px-5 py-3" style="font-family: 'IBM Plex Mono', monospace; color: var(--text-primary);">
									{ipset.routing.interfaces.join(', ')}
								</td>
							</tr>
						{/if}
						<tr style="border-bottom: 1px solid var(--stroke);">
